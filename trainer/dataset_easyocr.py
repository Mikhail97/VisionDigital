...
class Batch_Balanced_Dataset(object):
    ...
    def get_batch(self):
        ...
        for i, data_loader_iter in enumerate(self.dataloader_iter_list):
            try:
                image, text = next(data_loader_iter) #data_loader_iter.next()
                balanced_batch_images.append(image)
                balanced_batch_texts += text
            except StopIteration:
                self.dataloader_iter_list[i] = iter(self.data_loader_list[i])
                image, text = next(self.dataloader_iter_list[i]) #self.dataloader_iter_list[i].next()
                balanced_batch_images.append(image)
                balanced_batch_texts += text
            except ValueError:
                pass
...
class OCRDataset(Dataset):
    ...
    def __init__(self, root, opt):
        ...
        self.opt = opt
        #print(root)
        self.df = pd.read_csv(os.path.join(root,'labels.csv'), sep='^([^,]+),', engine='python', usecols=['filename', 'words'], keep_default_na=False, dtype=str) #добавил dtype=str
        self.df['words'] = self.df.words.astype(str)  #добавил
        ...
        if self.opt.data_filtering_off:
            ...
            for index in range(self.nSamples):
                ...
                try:
                    ...
                    pass #print('except = ',label)
                out_of_char = f'[^{self.opt.character}]'
                if re.search(out_of_char, str(label).lower()):  #добавил str
                    continue
    ...
    def __getitem__(self, index):
        ...
        if not self.opt.sensitive:
            label = str(label).lower()  #правил, str добавил

        # We only train and evaluate on alphanumerics (or pre-defined character set in train.py)
        out_of_char = f'[^{self.opt.character}]'
        label = re.sub(out_of_char, '', str(label))   #добавил str
